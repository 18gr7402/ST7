...
%% Klasse 2
 ...
 init_weight=ones(size(k2,1),1); % vores initierende vægtvektor, som opdateres løbende. Denne bliver resultatet til det optimalt placerede hyperplan
 weight=init_weight; % sørger for den initierende vægt opdateres
 count=1;
...
%% Klasse 3
 ...
 margin3=100; % vi starter med en margin på 0
 learningRate3=0.01; % learningRate er hvor mange/hurtigt vi går frem ad gangen, når vi skal finde den optimale placering af hyperplanet
 
 init_weight=ones(size(k3,1),1); % vores initierende vægtvektor, som opdateres løbende. Denne bliver resultatet til det optimalt placerede hyperplan
 weight=init_weight; % sørger for den initierende vægt opdateres
 count=1;
 ...
 while(1)
    ...
    [Y,I]=find(g<=margin3); % vi er kun interesseret i I her. Det I returnerer er de placeringer hvor diskriminant funktionen (g) er større eller lig med margin (som vi har sat til 0).
    
%% % gradient of perceptron criteria function
    
    critFunc=sum(-(weight'*y3(:,I)),2); % y(:,I) --> alle dem fra y der ligger på en fejlklassificeret palds i I %2-tallet er kun til for at gøre koden universel, fordi vi gerne ellers ville have den på en søjleform
    y_sum=sum(y3(:,I),2);
...
subplot(339);plot(critFunc3(:,1:count-2)');title('Value of criterion function - class 3')
 ...
 %% træn modellen / classify the unknown samples 
